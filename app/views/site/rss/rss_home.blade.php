	...
	<channel>
		...
		@if(isset($news))
			@foreach($news as $value)
				<?php $typeSlug = TypeNew::find($value->type_new_id)->slug; ?>
				<item>
					<title>
						<![CDATA[
						{{ $value->title }}
					...
					<description>
						<![CDATA[
						<a href="{{ url().'/'.$typeSlug.'/'.$value->slug }}">
					...
					<link>
						{{ url().'/'.$typeSlug.'/'.$value->slug }}
					</link>
					<pubDate>{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $value->start_date)->format('Y-m-d H:i') }}</pubDate>
					<guid>
						{{ url().'/'.$typeSlug.'/'.$value->slug }}
					</guid>
					<atom:link href="{{ url().'/'.$typeSlug.'/'.$value->slug }}" rel="self" type="application/rss+xml"/>
